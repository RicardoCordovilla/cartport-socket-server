...
export async function printAirportTicket( data: AirportTicketData): Promise<void> {
  ...
  const ticketContent = Buffer.concat([
    ...
    Buffer.from(
      `TOTAL GRABADO           ${data.subtotal.toFixed(2)} $\n`,
      "ascii"
    ),
    Buffer.from(
      `IVA   ${data.taxRate}%                ${data.tax.toFixed(2)} $\n`,
      "ascii"
    ),
    Buffer.from(
    ...
    ...(data.changeError !== undefined
      ? [
          Buffer.from(
            `Error de cambio:        ${data.changeError.toFixed(2)} $\n`,
            "ascii"
          ),
        ]
      : []),
...
export async function printFillTicket(data: FillTicketData): Promise<void> {
  ...
  const ticketContent = Buffer.concat([
    ...
    LEFT,
    Buffer.from(
      `Numero de maquina        :  ${data.stationId.toString().padStart(10)}\n`,
      "ascii"
    ),
    ...
    Buffer.from("\n", "ascii"),
    Buffer.from(
      `LLENADO DEL HOPPER     : ${data.amount.toFixed(2).padStart(8)} $\n`,
      "ascii"
    ),
...
export async function printCoinEmptyTicket(
  data: CoinEmptyTicketData
): Promise<void> {
  ...
  const ticketContent = Buffer.concat([
    ...
    Buffer.from("\n", "ascii"),
    Buffer.from(
      `MONTANTE EN HOPPER     : ${data.amount.toFixed(2).padStart(8)} $\n`,
      "ascii"
    ),
...
export async function printBillEmptyTicket(
  data: BillEmptyTicketData
): Promise<void> {
  ...
  const ticketContent = Buffer.concat([
    ...
    Buffer.from("\n", "ascii"),
    Buffer.from(
      `BILLETES  1                     ${data.bills1.toString().padStart(8)}\n`,
      "ascii"
    ),
    Buffer.from(
      `BILLETES  5                     ${data.bills5.toString().padStart(8)}\n`,
      "ascii"
    ),
    Buffer.from(
      `BILLETES 10                     ${data.bills10
        .toString()
        .padStart(8)}\n`,
      "ascii"
    ),
    Buffer.from("\n", "ascii"),
    Buffer.from(
      `MONTANTE EN BILLETERO   : ${data.totalAmount
        .toFixed(2)
        .padStart(8)} $\n`,
      "ascii"
    ),
...
export async function printRecaudacionTicket(
  data: RecaudacionTicketData
): Promise<void> {
  ...
  const ticketContent = Buffer.concat([
    ...
    Buffer.from("--------------------------------\n", "ascii"),
    Buffer.from(
      `NUMERO BILLETES 1       :         ${data.resumen.totalOne}\n`,
      "ascii"
    ),
    Buffer.from(
      `NUMERO BILLETES 5       :         ${data.resumen.totalFive}\n`,
      "ascii"
    ),
    Buffer.from(
      `NUMERO BILLETES 10      :         ${data.resumen.totalTen}\n`,
      "ascii"
    ),
    Buffer.from("--------------------------------\n", "ascii"),
    Buffer.from(
    ...
    Buffer.from(
      `MONEDAS EN HOPPER        :    ${data.hopper.toFixed(2)} $\n`,
      "ascii"
    ),

    Buffer.from(
      `MONTANTE MONEDERO       :    ${data.resumen.totalCoins.toFixed(2)} $\n`,
      "ascii"
    ),

    // Buffer.from(`MONTANTE MONEDERO       :     1.00 $\n`, 'ascii'),
    Buffer.from(
      `MONEDAS DEVUELTAS       :    ${data.resumen.totalCoinsGiven.toFixed(
        2
      )} $\n`,
      "ascii"
    ),
    ...
    Buffer.from(
      `VENTA TOTAL        :   ${(
        data.resumen.totalCartsSoldSession * 3
      ).toFixed(2)} $\n`,
      "ascii"
    ),
    Buffer.from("--------------------------------\n", "ascii"),
